...
from harukatl.tl.functions.channels import CreateChannelRequest, EditPhotoRequest
...
async def asset_channel(
    ...
    *,
    ...
    if title.startswith("legacy-"):
        title = title.replace("legacy-", "haruka-")
    ...
    if _folder:
        ...
        try:
            folder = next(
                ...
                if not isinstance(folder, harukatl.tl.types.DialogFilterDefault)
...
async def asset_forum_topic(
    ...
    async def create_topic() -> ForumTopic:
        result = await client(
            CreateForumTopicRequest(
                peer=entity,
                title=title,
                icon_emoji_id=(icon_emoji_id if client.haruka_me.premium else None),
            )
        )
    ...
    forums_cache = db.pointer("haruka.forums", "forums_cache", {})
...
async def get_topic_id(db: "Database", topic_name: str) -> typing.Optional[int]:
    ...
    """
    try:
        forums_cache = db.get("haruka.forums", "forums_cache", {})
        return forums_cache.get("haruka-userbot", {}).get(topic_name)
    except Exception:
        return None
