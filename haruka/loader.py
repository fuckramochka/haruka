...
_external_context = contextvars.ContextVar(
    "haruka_external_module_origin", default=None
)
...
def _make_session_allowlist():
    ...
    allowed_callers = frozenset({f"{__package__}.modules.haruka_plugin_security"})
    ...
    try:
        match name:
            ...
            case s if s.startswith("herokutl"):
                # Heroku's Telethon fork → stock Telethon
                return native_import("telethon" + name[8:], *args, **kwargs)
            case s if s.startswith("hikkatl"):
                # Hikka's Telethon fork → stock Telethon
                return native_import("telethon" + name[7:], *args, **kwargs)
            case s if s.startswith("hikkalls"):
                return native_import(name, *args, **kwargs)
            case s if s.startswith(("hikka.", "heroku.", "ftg.", "geektg.")) or s in (
                "hikka",
                "heroku",
                "ftg",
                "geektg",
            ):
                # Hikka / Heroku / FTG / GeekTG core package → Haruka
                root_len = len(s.partition(".")[0])
                return native_import("haruka" + name[root_len:], *args, **kwargs)
...
def _scan_external_source(source: str) -> typing.List[str]:
    """
    Lightweight static analysis of externally loaded module source.
    Returns human-readable findings of potentially dangerous patterns.
    This is NOT a sandbox — it only informs the user.
    """
    import ast
    ...
    for node in ast.walk(tree):
        if isinstance(node, (ast.Import, ast.ImportFrom)):
            ...
            for name in names:
                root = name.split(".")[0]
                if root in {"ctypes", "pty", "cffi"}:
                    findings.append(f"suspicious import: {root}")
            ...
            if isinstance(func, ast.Name):
                ...
                if full in {"os.system", "os.popen", "os.execv", "os.execve"}:
                    findings.append(f"shell execution via {full}")
                elif full.startswith("ctypes."):
                    findings.append(f"native code loading via {full}")
                elif full == "builtins.eval":
                    findings.append("direct eval() call")
...
class Modules:
    ...
    def __init__(
        ...
        from .compat import sync_client_attributes

        sync_client_attributes(self.client)
    ...
    async def register_module(
        ...
        if _is_external_origin(origin):
            with contextlib.suppress(Exception):
                raw = spec.loader.get_data(spec.origin or "")
                if isinstance(raw, bytes):
                    raw = raw.decode("utf-8", errors="ignore")
                findings = _scan_external_source(raw)
                if findings:
                    module.__security_findings__ = findings
                    for finding in findings:
                        logger.warning(
                            "Security scan of %s: %s",
                            module_name,
                            finding,
                        )
    ...
    def register_commands(self, instance: Module):
        ...
        for _command, cmd in instance.haruka_commands.items():
    ...
    async def complete_registration(self, instance: Module):
        ...
        internalized = []
        try:
            internalized = self._db.get("HarukaPluginSecurity", "internalized", [])
        except Exception:
            internalized = []
