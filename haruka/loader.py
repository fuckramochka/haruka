...
from harukatl.tl.tlobject import TLObject
...
try:
    from .inline.core import InlineManager  # type: ignore
except Exception as inline_import_error:
    _INLINE_IMPORT_ERROR = inline_import_error

    class _DummyAsyncStop:
    ...
    class _DummyBot:
        ...
        @staticmethod
        async def _drain_awaitables(*args, **kwargs):
            for arg in args:
        ...
        async def __call__(self, *args, **kwargs):
            await self._drain_awaitables(*args, **kwargs)
            return None
    ...
    class InlineManager:  # type: ignore
        def __init__(self, *_args, **_kwargs):
            self.init_complete = False
            self.bot = _DummyBot()
            self._dp = _DummyAsyncStop()
            self._task = None
        ...
        async def register_manager(self, *args, **kwargs):
            logger.warning(
                "Inline backend is disabled (%s). Running without bot-api layer.",
                _INLINE_IMPORT_ERROR,
            )
            return False
        ...
        async def gallery(self, *_args, **_kwargs):
            return False

        async def query_gallery(self, *_args, **_kwargs):
            return False

        async def check_inline_security(self, *_args, **_kwargs):
            return False

        def pop_web_auth_token(self, *_args, **_kwargs):
            return False

        def build_pagination(self, *_args, **_kwargs):
            return []

        def generate_markup(self, *_args, **_kwargs):
            return None

        def _normalize_markup(self, *_args, **_kwargs):
            return None
...
PIP_REQUIREMENT_ALIASES = {
    "sklearn": "scikit-learn",
    "pil": "Pillow",
    "harukatl": "Haruka-TL-New",
    "hikkatl": "Haruka-TL-New",
    "herokutl": "Haruka-TL-New",
}
PIP_DIRECT_REFERENCE_PREFIXES = ("http://", "https://", "git+", "svn+", "hg+", "bzr+")
PIP_NAME_SPLIT_RE = re.compile(r"[<>=!~\[\]@;]")


def _extract_requirement_tokens(raw_requirements: str) -> typing.Iterable[str]:
    for requirement in map(str.strip, raw_requirements.split()):
        if (
            "," in requirement
            and "[" not in requirement
            and "]" not in requirement
            and not any(op in requirement for op in "<>=!~;@")
            and not requirement.lower().startswith(PIP_DIRECT_REFERENCE_PREFIXES)
        ):
            for chunk in map(str.strip, requirement.split(",")):
                if chunk:
                    yield chunk
            continue
...
def normalize_requirement(requirement: typing.Optional[str]) -> typing.Optional[str]:
    if not requirement:
        return None
    ...
    if lowered.startswith(PIP_DIRECT_REFERENCE_PREFIXES):
        return requirement

    name = PIP_NAME_SPLIT_RE.split(requirement, maxsplit=1)[0].strip()
    if not name:
        return None

    alias = PIP_REQUIREMENT_ALIASES.get(name.lower())
    if alias:
        requirement = alias + requirement[len(name) :]
        name = alias
...
def parse_requirements(code: str) -> typing.List[str]:
    match = VALID_PIP_PACKAGES.search(code)
    if not match:
        return []

    requirements = []
    for requirement in _extract_requirement_tokens(match[1]):
...
def patched_import(name: str, *args, **kwargs):
    if name.startswith("telethon"):
        return native_import("harukatl" + name[8:], *args, **kwargs)
    elif name.startswith("hikkatl"):
        return native_import("harukatl" + name[7:], *args, **kwargs)
    elif name.startswith("herokutl"):
        return native_import("harukatl" + name[8:], *args, **kwargs)
    elif name.startswith("hikkalls"):
        return native_import(name, *args, **kwargs)
    elif name.startswith("hikka"):
        return native_import("haruka" + name[5:], *args, **kwargs)
...
class InfiniteLoop:
    ...
    def stop(self, *args, **kwargs):
        with contextlib.suppress(AttributeError):
            _haruka_client_id_logging_tag = copy.copy(  # noqa: F841
    ...
    async def actual_loop(self, *args, **kwargs):
        ...
        background_ready = getattr(
            getattr(self.module_instance, "_client", None),
            "haruka_background_ready",
            None,
        )
        if isinstance(background_ready, asyncio.Event):
...
def tag(*tags, **kwarg_tags):
    ...
    @loader.tag("only_messages", "only_pm", regex=r"^[.] ?haruka$", from_id=659800858)
...
class Modules:
    ...
    def __init__(
        ...
        self.client.haruka_inline = self.inline
    ...
    async def _junk_collector(self):
        ...
        while True:
            ...
            for module in self.modules:
                commands.update(module.haruka_commands)
                inline_handlers.update(module.haruka_inline_handlers)
                callback_handlers.update(module.haruka_callback_handlers)
                watchers.extend(module.haruka_watchers.values())
    ...
    async def _register_modules(
        ...
        with contextlib.suppress(AttributeError):
            _haruka_client_id_logging_tag = copy.copy(self.client.tg_id)  # noqa: F841
    ...
    def register_raw_handlers(self, instance: Module):
        ...
        for name, handler in utils.iter_attrs(instance):
            # Raw handler marker is applied by @loader.raw_handler decorator
            if not callable(handler):
                continue

            if getattr(handler, "is_raw_handler", False) is not True:
                continue

            updates = getattr(handler, "updates", ())
            if not isinstance(updates, tuple):
                logger.debug(
                    "Skipping raw handler %s for %s: invalid updates spec %r",
                    name,
    ...
    def register_commands(self, instance: Module):
        ...
        if instance.__origin__.startswith("<core"):
            self._core_commands += list(
                map(lambda x: x.lower(), list(instance.haruka_commands))
            )

        skipped_core_conflicts = []

        for _command, cmd in list(instance.haruka_commands.items()):
            ...
            if (
                ...
                skipped_core_conflicts.append(_command)
                instance.haruka_commands.pop(_command, None)
                continue

            self.commands.update({_command.lower(): cmd})

        if skipped_core_conflicts:
            logger.warning(
                "Skipped core-conflicting command(s) for %s: %s",
                instance.__class__.__name__,
                ", ".join(skipped_core_conflicts),
            )

        for alias, cmd in self.aliases.copy().items():
            _cmd = cmd.split(maxsplit=1)
            if _cmd[0] in instance.haruka_commands:
                self.add_alias(alias, *_cmd)

        self.register_inline_stuff(instance)

    def register_inline_stuff(self, instance: Module):
        for name, func in instance.haruka_inline_handlers.copy().items():
        ...
        for name, func in instance.haruka_callback_handlers.copy().items():
    ...
    def register_watchers(self, instance: Module):
        ...
        for _watcher in instance.haruka_watchers.values():
