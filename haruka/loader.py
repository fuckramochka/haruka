    ...
    try:
        match name:
            ...
            case s if s.startswith("herokutl"):
                # legacy Heroku plugins -> Haruka TL alias (redirects to herokutl)
                return native_import("harukatl" + name[8:], *args, **kwargs)
            case s if s.startswith("hikkalls"):
                return native_import(name, *args, **kwargs)
            case s if s.startswith("hikka"):
                return native_import("haruka" + name[5:], *args, **kwargs)
            case s if s.startswith("heroku") and not s.startswith("heroku-"):
                # legacy Heroku plugins -> Haruka core namespace
                return native_import("haruka" + name[6:], *args, **kwargs)
