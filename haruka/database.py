...
from harukatl.tl.types import Message, User
...
_DB_PROTECTED_OWNERS = {"HarukaPluginSecurity"}
_DB_ALLOWED_WRITERS = {f"{__package__}.modules.haruka_plugin_security"}
...
class Database(dict):
    def __init__(self, client: CustomTelegramClient):
        ...
        # Debounced disk-flush task: coalesces bursts of set() calls into a
        # single write instead of serialising the whole DB on every change.
        self._file_save_task: asyncio.Future = None
    ...
    def _file_save_sync(self, snapshot: dict):
        """Atomically persist a snapshot of the DB to disk."""
        tmp = self._db_file.with_suffix(".json.tmp")
        tmp.write_bytes(_dumps(snapshot))
        os.replace(tmp, self._db_file)

    async def _file_save(self) -> bool:
        """Debounced background flush to disk."""
        try:
            while self._file_dirty:
                self._file_dirty = False
                await asyncio.sleep(1)
                await utils.run_sync(self._file_save_sync, dict(self))
                logger.debug("Flushed db to disk")
        except Exception:
            logger.exception("Database save failed!")
            return False
        finally:
            self._file_save_task = None
        return True
    ...
    def read(self):
        ...
        try:
            ...
            for legacy in ("hikka", "legacy", "heroku"):
                if re.search(rf'"({legacy}\.)(\S+\":)', db):
                    logging.warning("Converting db after update (%s -> haruka)", legacy)
                    db = re.sub(
                        rf"({legacy}\.)(\S+\":)",
                        lambda m: "haruka." + m.group(2),
                        db,
                    )
            self._update_from_read(_loads(db))
    ...
    def save(self) -> bool:
        ...
        if loop is not None:
            # Coalesce rapid writes: one debounced flush instead of one
            # full serialisation + fsync per set() call.
            if not self._file_save_task or self._file_save_task.done():
                self._file_save_task = asyncio.ensure_future(self._file_save())
            return True

        # No event loop yet (early startup): fall back to a direct write.
        try:
            self._file_dirty = False
            self._file_save_sync(dict(self))
        except Exception:
            logger.exception("Database save failed!")
            return False
    ...
    async def store_asset(self, message: Message) -> int:
        ...
        try:
            _assets_topic_id = self.get("haruka.forums", "forums_cache", {})[
                "haruka-userbot"
            ]["Assets"]
        except (TypeError, KeyError):
            raise NoAssetsChannel("Tried to save asset to non-existing asset topic.")

        if not (_content_channel_id := self.get("haruka.forums", "channel_id", None)):
