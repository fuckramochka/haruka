...
class Database(dict):
    ...
    def read(self):
        ...
        try:
            ...
            for legacy_prefix in ("hikka", "heroku", "legacy"):
                pattern = rf'"{legacy_prefix}\.([^"]+)":'
                if re.search(pattern, db):
                    logging.warning(
                        "Migrating \"%s.*\" database keys to \"haruka.*\"", legacy_prefix
                    )
                    db = re.sub(pattern, lambda m: f'"haruka.{m.group(1)}":', db)
