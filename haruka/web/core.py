...
class Web(root.Web):
    ...
    async def get_url(self, proxy_pass: bool) -> str:
        ...
        if not url:
            ...
            ip = os.environ.get("HARUKA_IP", ip)
    ...
    async def start(self, port: int, proxy_pass: bool = False):
        ...
        self.running.set()
        print(f"Haruka Userbot Web Interface running on {self.port}")
    ...
    @staticmethod
    async def favicon(_):
        return web.FileResponse("web-resources/static/haruka-favicon.png")

    @staticmethod
    async def healthz(_):
        """Lightweight liveness probe for orchestrators (Docker/K8s)"""
