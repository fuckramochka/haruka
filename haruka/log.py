...
from harukatl.errors import PersistentTimestampOutdatedError
from harukatl.errors.rpcbaseerrors import (
    ServerError,
    RPCError
)
...
def getlines(filename: str, module_globals=None) -> str:
    ...
    source code of Haruka modules properly. This is needed for
    ...
    try:
        if filename.startswith("<") and filename.endswith(">"):
            ...
            if (module.startswith("haruka.modules")) and module in sys.modules:
                return list(
                    map(
                        lambda x: f"{x}\n",
                ...
                if isinstance(value, dict):
                    ...
                    try:
                        if (
                            ...
                            value,
                            (harukatl.TelegramClient, CustomTelegramClient),
                        ):
...
class TelegramLogsHandler(logging.Handler):
    ...
    def dumps(
        ...
        return [
            ...
            and (not record.haruka_caller or client_id == record.haruka_caller)
        ]
    ...
    async def _show_full_trace(
        ...
        item: HarukaException,
    ):
        chunks = item.message + "\n\n<b>🪐 Full traceback:</b>\n" + item.full_stack

        chunks = list(utils.smart_split(*harukatl.extensions.html.parse(chunks), 4096))
    ...
    def _gen_web_debug_button(self, item: HarukaException) -> list:
        if not item.sysinfo:
            return []
    ...
    def get_logid_by_client(self, client_id: int) -> typing.Optional[int]:
        mod = self._mods.get(client_id)
        if not mod:
            return None

        return getattr(mod, "logchat", None)
        ...
        async with self._send_lock:
            ...
            self._exc_queue = {
                client_id: [
                    ...
                    for item in self.tg_buff
                    if isinstance(item[0], HarukaException)
    ...
    def emit(self, record: logging.LogRecord):
        try:
            caller = next(
                (
                    frame_info.frame.f_locals["_haruka_client_id_logging_tag"]
                    for frame_info in inspect.stack()
                    if isinstance(
                        getattr(getattr(frame_info, "frame", None), "f_locals", {}).get(
                            "_haruka_client_id_logging_tag"
                        ),
                        int,
                    )
                ),
        ...
        if record.levelno >= self.tg_level:
            if record.exc_info:
                ...
                exc = HarukaException.from_exc_info(
                    *record.exc_info,
...
def init():
    ...
    logging.getLogger("harukatl").setLevel(logging.WARNING)
