...
class StringLoader(SourceLoader):
    ...
    def get_code(self, fullname: str):
        ...
        # Bytecode cache: modules are loaded from strings (no .pyc on disk), so
        # by default Python re-parses+compiles every module on every boot.
        # Cache compiled code objects keyed by source hash to make repeated
        # startups dramatically faster. Falls back to plain compile on any error.
        try:
            import hashlib
            import marshal
            import tempfile
            ...
            cache_dir = os.path.join(tempfile.gettempdir(), "haruka_bytecode_cache")
            os.makedirs(cache_dir, exist_ok=True)
            cache_file = os.path.join(cache_dir, f"{key}-{ver}.hbc")
...
class Module:
    ...
    async def request_join(
        ...
        match channel:
            ...
            case _ if channel.id in self._db.get("haruka.main", "declined_joins", []):
                if assure_joined:
                    raise LoadError(
...
class CacheRecordEntity:
    __slots__ = ("entity", "_hashable_entity", "_exp", "ts")
...
class CacheRecordPerms:
    __slots__ = ("perms", "_hashable_entity", "_hashable_user", "_exp", "ts")
