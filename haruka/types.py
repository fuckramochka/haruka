...
HarukaReplyMarkup = typing.Union[typing.List[typing.List[dict]], typing.List[dict], dict]
...
class Module:
    ...
    @property
    def haruka_commands(self) -> typing.Dict[str, Command]:
        """List of commands that module supports"""
        return get_commands(self)
    ...
    @property
    def haruka_inline_handlers(self) -> typing.Dict[str, Command]:
        """List of inline handlers that module supports"""
        return get_inline_handlers(self)
    ...
    @property
    def haruka_callback_handlers(self) -> typing.Dict[str, Command]:
        """List of callback handlers that module supports"""
        return get_callback_handlers(self)
    ...
    @property
    def haruka_watchers(self) -> typing.Dict[str, Command]:
        """List of watchers that module supports"""
        return get_watchers(self)
    ...
    @haruka_commands.setter
    def haruka_commands(self, _):
        pass
    ...
    @haruka_inline_handlers.setter
    def haruka_inline_handlers(self, _):
        pass
    ...
    @haruka_callback_handlers.setter
    def haruka_callback_handlers(self, _):
        pass
    ...
    @haruka_watchers.setter
    def haruka_watchers(self, _):
        pass
    ...
    async def _decline(
        ...
        self._db.set(
            "haruka.main",
            "declined_joins",
            list(set(self._db.get("haruka.main", "declined_joins", []) + [channel.id])),
        )
        event.status = False
        event.set()
        await call.edit(
            ...
            photo="https://raw.githubusercontent.com/coddrago/assets/refs/heads/main/haruka/declined_jr.png",
        )
    ...
    async def request_join(
        ...
        try:
            ...
            if assure_joined:
                raise LoadError(
                    f"You need to join @{peer} in order to use this module, "
                    "but this peer is unavailable"
                ) from e

            logger.debug(
                "Skipping join request for %r: unable to resolve peer (%s)",
                peer,
                e,
            )
            return False
        ...
        if channel.id in self._db.get("haruka.main", "declined_joins", []):
            if assure_joined:
                raise LoadError(
        ...
        if not getattr(self.inline, "bot_username", None):
            if assure_joined:
                raise LoadError(
                    f"You need to join @{channel.username or peer} in order to use this module, "
                    "but inline bot is unavailable to request confirmation"
                )

            logger.debug(
                "Skipping join request for %s: inline bot is unavailable",
                channel.username or peer,
            )
            return False
        ...
        join_markup = self.inline.generate_markup(
            [
                {
                    "text": "💫 Approve",
        ...
        try:
            await self.inline.bot.send_photo(
                self.tg_id,
                "https://raw.githubusercontent.com/coddrago/assets/refs/heads/main/haruka/join_request.png",
                caption=join_caption,
                reply_markup=join_markup,
            )
        except Exception as e:
            logger.debug(
                "Failed to send join request photo, falling back to text: %s",
                e,
            )
        ...
        self.haruka_wait_channel_approve = (
            self.__class__.__name__,
            channel,
            reason,
        )
        ...
        with contextlib.suppress(AttributeError):
            delattr(self, "haruka_wait_channel_approve")
    ...
    async def import_lib(
        ...
        *,
        ...
        from .loader import USER_INSTALL, normalize_requirement, parse_requirements
        ...
        if re.search(r"# ?scope: ?haruka_min", code):
            ver = tuple(
                map(
                    int,
                    re.search(r"# ?scope: ?haruka_min ((\d+\.){2}\d+)", code)[1].split(
                        "."
                    ),
                )
            )
            ...
            if version.__version__ < ver:
                _raise(
                    RuntimeError(
                        f"Library requires Haruka version {'{}.{}.{}'.format(*ver)}+"
                    )
                )

        module = f"haruka.libraries.{url.replace('%', '%%').replace('.', '%d')}"
        ...
        try:
            ...
            if not requirements:
                ...
                requirement = normalize_requirement(e.name)
        ...
        if (
            ...
            and self._db.get("haruka.main", "stats", True)
            and url is not None
