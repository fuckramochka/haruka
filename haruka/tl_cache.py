...
from harukatl.errors import RPCError
...
class CustomTelegramClient(TelegramClient):
    ...
    async def get_perms_cached(
        ...
        if resolved_perms:
            ...
            self._haruka_perms_cache.setdefault(hashable_entity, {})[hashable_user] = (
                cache_record
            )
            ...
            def save_user(key: typing.Union[str, int]):
                ...
                if getattr(user, "username", None):
                    self._haruka_perms_cache.setdefault(key, {})[f"@{user.username}"] = (
                        cache_record
                    )
                    self._haruka_perms_cache.setdefault(key, {})[user.username] = (
                        cache_record
                    )
