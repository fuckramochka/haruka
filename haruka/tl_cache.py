...
from harukatl import TelegramClient
from harukatl import __name__ as __base_name__
from harukatl import helpers
from harukatl._updates import ChannelState, Entity, EntityType, SessionState
from harukatl.errors.rpcerrorlist import TopicDeletedError
from harukatl.hints import EntityLike
from harukatl.network import MTProtoSender
from harukatl.tl import functions
from harukatl.tl.alltlobjects import LAYER
from harukatl.tl.functions.channels import GetFullChannelRequest
from harukatl.tl.functions.users import GetFullUserRequest
from harukatl.tl.tlobject import TLRequest
from harukatl.tl.types import (
    ChannelFull,
    Message,
    Updates,
    UpdatesCombined,
...
from harukatl.utils import is_list_like
...
# Upper bound for the entity cache to prevent unbounded memory growth on
# long-running sessions. Expired records are dropped first, then oldest by ts.
_MAX_ENTITY_CACHE = 1500
...
class CustomTelegramClient(TelegramClient):
    def __init__(self, *args, **kwargs):
        ...
        self._haruka_perms_cache: typing.Dict[
            typing.Union[str, int],
            CacheRecordPerms,
        ] = {}

        self._haruka_fullchannel_cache: typing.Dict[
            typing.Union[str, int],
            CacheRecordFullChannel,
        ] = {}

        self._haruka_fulluser_cache: typing.Dict[
            typing.Union[str, int],
            CacheRecordFullUser,
        ] = {}
        ...
        self.haruka_me: "User"
        self.hikka_me: "User"
        self.haruka_db: "Database"
        self.loader: "Modules"
        self.haruka_inline: "InlineManager"
    ...
    @property
    def haruka_entity_cache(self) -> typing.Dict[int, CacheRecordEntity]:
        return self._haruka_entity_cache

    def _prune_entity_cache(self) -> None:
        """Drop expired records, then cap the entity cache size (oldest by ts)."""
        cache = self._haruka_entity_cache
        try:
            expired = [
                k for k, v in list(cache.items()) if getattr(v, "expired", False)
            ]
            for k in expired:
                cache.pop(k, None)
            overflow = len(cache) - _MAX_ENTITY_CACHE
            if overflow > 0:
                oldest = sorted(
                    cache.items(), key=lambda kv: getattr(kv[1], "ts", 0)
                )
    ...
    @property
    def haruka_perms_cache(self) -> typing.Dict[int, CacheRecordPerms]:
        return self._haruka_perms_cache

    @property
    def haruka_fullchannel_cache(self) -> typing.Dict[int, CacheRecordFullChannel]:
        return self._haruka_fullchannel_cache

    @property
    def haruka_fulluser_cache(self) -> typing.Dict[int, CacheRecordFullUser]:
        return self._haruka_fulluser_cache
    ...
    async def get_entity(
        ...
        _haruka_client_id_logging_tag = copy.copy(self.tg_id)  # noqa: F841
        ...
        if (
            not force
            and hashable_entity
            and hashable_entity in self._haruka_entity_cache
            and (
                not exp
                or self._haruka_entity_cache[hashable_entity].ts + exp > time.time()
            )
        ):
            logger.debug(
                "Using cached entity %s (%s)",
                entity,
                type(self._haruka_entity_cache[hashable_entity].entity).__name__,
            )
            return copy.deepcopy(self._haruka_entity_cache[hashable_entity].entity)

        resolved_entity = await super().get_entity(entity)

        if resolved_entity:
            cache_record = CacheRecordEntity(hashable_entity, resolved_entity, exp)
            self._haruka_entity_cache[hashable_entity] = cache_record
            ...
            if getattr(resolved_entity, "id", None):
                ...
                self._haruka_entity_cache[resolved_entity.id] = cache_record
            ...
            if getattr(resolved_entity, "username", None):
                ...
                self._haruka_entity_cache[f"@{resolved_entity.username}"] = cache_record
                self._haruka_entity_cache[resolved_entity.username] = cache_record
    ...
    async def get_perms_cached(
        ...
        if (
            ...
            and hashable_entity
            and hashable_user
            and hashable_user in self._haruka_perms_cache.get(hashable_entity, {})
            and (
                not exp
                or self._haruka_perms_cache[hashable_entity][hashable_user].ts + exp
                > time.time()
            )
        ):
            logger.debug("Using cached perms %s (%s)", hashable_entity, hashable_user)
            return copy.deepcopy(
                self._haruka_perms_cache[hashable_entity][hashable_user].perms
            )
        ...
        if resolved_perms:
            ...
            self._haruka_perms_cache.setdefault(hashable_entity, {})[
                hashable_user
            ] = cache_record
            ...
            def save_user(key: typing.Union[str, int]):
                ...
                if getattr(user, "id", None):
                    self._haruka_perms_cache.setdefault(key, {})[user.id] = cache_record

                if getattr(user, "username", None):
                    self._haruka_perms_cache.setdefault(key, {})[
                        f"@{user.username}"
                    ] = cache_record
                    self._haruka_perms_cache.setdefault(key, {})[
                        user.username
    ...
    async def get_fullchannel(
        ...
        if (
            not force
            and self._haruka_fullchannel_cache.get(hashable_entity)
            and not self._haruka_fullchannel_cache[hashable_entity].expired
            and self._haruka_fullchannel_cache[hashable_entity].ts + exp > time.time()
        ):
            return self._haruka_fullchannel_cache[hashable_entity].full_channel

        result = await self(GetFullChannelRequest(channel=entity))
        self._haruka_fullchannel_cache[hashable_entity] = CacheRecordFullChannel(
            hashable_entity,
            result,
            exp,
        )
    ...
    async def get_fulluser(
        ...
        if (
            not force
            and self._haruka_fulluser_cache.get(hashable_entity)
            and not self._haruka_fulluser_cache[hashable_entity].expired
            and self._haruka_fulluser_cache[hashable_entity].ts + exp > time.time()
        ):
            return self._haruka_fulluser_cache[hashable_entity].full_user

        result = await self(GetFullUserRequest(entity))
        self._haruka_fulluser_cache[hashable_entity] = CacheRecordFullUser(
            hashable_entity,
            result,
            exp,
        )
    ...
    async def _call(
        ...
        # list and you will be banned from Haruka federation.
