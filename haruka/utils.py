...
import grapheme
import harukatl
import harukatl.extensions
import harukatl.extensions.html
import requests
from aiogram.types import Message as AiogramMessage
from harukatl import hints
from harukatl.errors.rpcbaseerrors import RPCError
from harukatl.tl.custom.message import Message
from harukatl.tl.functions.account import UpdateNotifySettingsRequest
from harukatl.tl.functions.channels import (
...
try:
    ...
    class InlineMessage:  # type: ignore
        pass

    class InlineCall:  # type: ignore
        pass

    class BotInlineCall(InlineCall):  # type: ignore
        pass
from .tl_cache import CustomTelegramClient
from .types import HarukaReplyMarkup, ListLike, Module
...
parser = harukatl.utils.sanitize_parse_mode("html")
...
def get_chat_id(message: typing.Union[Message, AiogramMessage]) -> int:
    ...
    :return: Chat ID
    """
    return harukatl.utils.resolve_id(
...
def get_entity_id(entity: hints.Entity) -> int:
    ...
    :return: Entity ID
    """
    return harukatl.utils.get_peer_id(entity)
...
async def answer(
    ...
    response: str,
    *,
    reply_markup: typing.Optional[HarukaReplyMarkup] = None,
    **kwargs,
) -> typing.Union[InlineCall, InlineMessage, Message]:
    """
    Use this to give the response to a command
    :param message: Message to answer to. Can be a tl message or haruka inline object
    ...
    parse_mode = harukatl.utils.sanitize_parse_mode(
        kwargs.pop(
            "parse_mode",
    ...
    if isinstance(response, str) and not kwargs.pop("asfile", False):
        ...
        if "<blockquote expandable" in response.lower():
            for entity in entities:
                if isinstance(entity, MessageEntityBlockquote):
                    with contextlib.suppress(Exception):
                        entity.collapsed = True

        if len(text) >= 4096 and not hasattr(message, "haruka_grepped"):
            try:
...
async def set_avatar(
    ...
    if isinstance(avatar, str) and check_url(avatar):
        try:
            ...
            logger.warning("Failed to download avatar from %s", avatar, exc_info=True)
            return False

        if not response.ok:
            logger.warning(
                "Failed to download avatar from %s (HTTP %s)",
                avatar,
        ...
        if content_type and "image" not in content_type.lower():
            logger.warning(
                "Avatar URL %s does not look like an image (%s)",
                avatar,
                content_type,
            )
            return False
    ...
    try:
        ...
        logger.warning("Skipping avatar update for %s: %s", peer, e)
        return False
    except Exception:
        logger.warning("Unexpected error while updating avatar for %s", peer, exc_info=True)
        return False
...
async def invite_inline_bot(
    ...
    loader_obj = getattr(client, "loader", None)
    inline_obj = getattr(loader_obj, "inline", None)
    bot_username = getattr(inline_obj, "bot_username", None)
    if not bot_username:
...
async def asset_channel(
    ...
    *,
    ...
    cache_ttl = 60 * 60
    db = getattr(client, "haruka_db", None)

    def _cache_channel(peer: Channel):
        client._channels_cache[title] = {"peer": peer, "exp": int(time.time() + cache_ttl)}

        if not db:
            return

        with contextlib.suppress(Exception):
            if not isinstance(getattr(peer, "id", None), int):
                return

            persistent = db.get(__name__, "asset_channels", {}) or {}
            persistent_id = int(f"-100{peer.id}")
            if persistent.get(title) == persistent_id:
                return

            persistent[title] = persistent_id
            db.set(__name__, "asset_channels", persistent)
    ...
    # legacy haruka / hikka chats conversion to haruka
    if title.startswith("hikka-"):
        title = title.replace("hikka-", "haruka-")

    if db:
        with contextlib.suppress(Exception):
            persistent = db.get(__name__, "asset_channels", {}) or {}
            cached_peer = persistent.get(title)
            if cached_peer:
                try:
                    peer = await client.get_entity(cached_peer)
                except Exception:
                    persistent.pop(title, None)
                    db.set(__name__, "asset_channels", persistent)
                else:
                    _cache_channel(peer)
                    return peer, False
    ...
    loader_obj = getattr(client, "loader", None)
    inline_obj = getattr(loader_obj, "inline", None)
    inline_bot_username = getattr(inline_obj, "bot_username", None)
    inline_bot_id = getattr(inline_obj, "bot_id", None)
    if invite_bot and not inline_bot_username:
        logger.warning(
            "Skipping inline bot invite for '%s': inline bot is unavailable",
            title,
        )
        invite_bot = False
    ...
    async for d in client.iter_dialogs():
        if d.title == title:
            _cache_channel(d.entity)
            if invite_bot:
                needs_invite = True
                if inline_bot_id is not None:
                    needs_invite = all(
                        participant.id != inline_bot_id
                        for participant in (
    ...
    if _folder:
        ...
        try:
            folder = next(folder for folder in folders if folder.title == "haruka")
        except Exception:
            folder = None
...
def smart_split(
    ...
    :example:
        >>> utils.smart_split(
            *harukatl.extensions.html.parse(
                "<b>Hello, world!</b>"
            )
        )
        <<< ["<b>Hello, world!</b>"]
...
def get_git_hash() -> str:
    ...
    :return: Git commit hash or empty string if unavailable
    """
    try:
...
def get_commit_url() -> str:
    ...
    try:
        ...
        if not hash_:
            return "Unknown"
        return f'<a href="https://github.com/fuckramochka/haruka/commit/{hash_}">#{hash_[:7]}</a>'
    except Exception:
        return "Unknown"
...
def validate_html(html: str) -> str:
    ...
    :return: Valid HTML
    """
    text, entities = harukatl.extensions.html.parse(html)
    return harukatl.extensions.html.unparse(escape_html(text), entities)
...
def get_git_info() -> typing.Tuple[str, str]:
    ...
    return (
        hash_,
        f"https://github.com/fuckramochka/haruka/commit/{hash_}" if hash_ else "",
    )
