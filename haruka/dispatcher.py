...
class CommandDispatcher:
    def __init__(
        ...
        self._me = self._client.haruka_me.id
        self._cached_usernames = set()

        if self._client.haruka_me.username:
            self._cached_usernames.add(self._client.haruka_me.username.lower())

        if self._client.haruka_me.usernames:
            self._cached_usernames.update(
                u.username.lower()
                for u in getattr(self._client.haruka_me, "usernames", [])
            )

        self._cached_usernames.add(str(self._client.haruka_me.id))
    ...
    async def _handle_command(
        ...
        # Unknown command: stay silent by default. Suggestions are strictly
        # opt-in (db key `suggest_commands`) so plain dotted text never
        # triggers a response.
        if (
            not func
            and event.out
            and self._db.get(main.__name__, "suggest_commands", False)
            and " " not in tag[0]
        ):
            await self._suggest_command(message, tag[0])
    ...
    async def _suggest_command(self, message: Message, command: str):
        """
        Suggest a close existing command for a single-word unknown command.
        Only called when the user explicitly opted in via `suggest_commands`.
        """
        now = time.time()
        if now - getattr(self, "_last_suggestion_ts", 0) < 30:
            return
        self._last_suggestion_ts = now

        candidates = set(self._modules.commands)
        candidates.update(self._modules.aliases)
        matches = difflib.get_close_matches(command.lower(), candidates, n=2, cutoff=0.70)

        if not matches:
            return

        prefix = self._modules.get_prefix(utils.get_chat_id(message)) or "."
        suggestions = " ".join(
            f"<code>{utils.escape_html(prefix + m)}</code>" for m in sorted(matches)
        )

        with contextlib.suppress(Exception):
            await message.respond(
                f"🤔 <b>Unknown command</b> <code>{utils.escape_html(command)}</code>."
