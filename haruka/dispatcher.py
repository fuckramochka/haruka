...
from harukatl.errors import FloodWaitError, RPCError
...
class CommandDispatcher:
    def __init__(
        ...
        self._cached_usernames = [
            (
                self._client.haruka_me.username.lower()
                if self._client.haruka_me.username
                else str(self._client.haruka_me.id)
            )
        ]
    ...
    async def handle_raw(self, event: events.Raw):
        ...
        for handler in self.raw_handlers:
            ...
            if not isinstance(updates, tuple):
                logger.warning(
                    "Skipping raw handler %s: invalid updates spec %r",
                    getattr(handler, "id", handler),
                    updates,
                )
                continue

            try:
                matches = isinstance(event, updates)
            except TypeError:
                logger.warning(
                    "Skipping raw handler %s: malformed update types %r",
                    getattr(handler, "id", handler),
                    updates,
                )
                continue
