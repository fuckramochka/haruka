...
from . import _internal as _internal_state
...
def parse_arguments() -> dict:
    ...
    parser.add_argument(
        ...
        dest="dry_run",
        action="store_true",
        help="Validate config and core modules, then exit without connecting",
    )
...
class Haruka:
    ...
    async def save_client_session(
        ...
        *,
        ...
        try:
            db = client.haruka_db
            existing = db.get("haruka.inline", "custom_bot", False)
        except Exception:
            existing = False
        ...
        if (
            ...
            while bot := input(
                "You can enter a custom bot username or leave it empty and Haruka will generate a random one: "
            ):
                bot = bot.strip()
                bot = bot.lstrip("@")
                if any(
    ...
    async def _check_bot(
        ...
        url: str = (
            await client(
                telethon.functions.messages.RequestWebViewRequest(
                    peer="@botfather",
                    bot="@botfather",
                    platform="android",
                    from_bot_menu=False,
    ...
    async def _badge(self, client: CustomTelegramClient):
        ...
        try:
            if os.environ.get("HARUKA_NO_GIT") == "1":
                build = "unknown"
                upd = "Git disabled"
            else:
                import git
            ...
            pref = client.haruka_db.get("haruka.main", "command_prefix", None)
            ...
            try:
                ...
                await client.haruka_inline.bot.send_message(
                    log_chat_id,
                    (
                        "<b>🌸 Haruka {} — online</b>\n"
                        "<i>independent · compatible · fast</i>\n"
                        "\n"
                        "├ <b>Build:</b> <code>{}</code>\n"
                        "├ <b>Status:</b> {}\n"
                        "{}"
    ...
    async def _dry_run(self):
        """Offline validation: config, core modules and syntax — no connection"""
        print("🌸 Haruka dry-run validation")
        ok = True
        ...
        if CONFIG_PATH.exists():
            try:
                ...
                ok = False
                print(f"  [FAIL] config.json is broken: {e}")
        else:
            print("  [WARN] config.json not found (first run?)")

        modules_dir = Path(utils.get_base_dir()) / "modules"
        if modules_dir.is_dir():
            sources = list(modules_dir.glob("*.py"))
            failed = []
            for src in sources:
                try:
            ...
            print(
                f"  [OK]   {len(sources) - len(failed)}/{len(sources)}"
            ...
            print("  [FAIL] core modules directory not found")
            ok = False

        loaded_dir = loader.LOADED_MODULES_DIR
        external = [
            f
            for f in os.listdir(loaded_dir)
            if f.endswith(".py") and not f.startswith("_")
        ] if os.path.isdir(loaded_dir) else []
        print(f"  [INFO] external modules on disk: {len(external)}")
    ...
    async def _main(self):
        """Main entrypoint"""
        if getattr(self.arguments, "dry_run", False):
            await self._dry_run()
            return
...
_internal_state.update_feature_flags(CONFIG_PATH)
