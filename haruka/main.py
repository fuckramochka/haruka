...
import aiohttp
import harukatl
from harukatl import events
from harukatl.errors import (
    ApiIdInvalidError,
...
from harukatl.network.connection import (
    ConnectionTcpFull,
    ConnectionTcpMTProxyRandomizedIntermediate,
)
from harukatl.password import compute_check
from harukatl.sessions import MemorySession, SQLiteSession
from harukatl.tl.functions.account import GetPasswordRequest
from harukatl.tl.functions.auth import CheckPasswordRequest
...
class Haruka:
    ...
    def __init__(self):
        ...
        try:
            ...
            # Python 3.14+: there is no implicit loop in main thread.
    ...
    def _read_sessions(self):
        ...
        for filename in os.listdir(BASE_DIR):
            ...
            if not (filename.startswith("haruka-") or filename.startswith("hikka-")):
                continue

            # Skip duplicate basenames that may appear from stale sqlite sidecar files.
            session_name = filename.rsplit(".session", maxsplit=1)[0]
        ...
        self.sessions += [
            SQLiteSession(os.path.join(BASE_DIR, session_name))
    ...
    async def save_client_session(
        ...
        *,
        ...
        client.haruka_db = database.Database(client)
        await client.haruka_db.init()
    ...
    async def _phone_login(self, client: CustomTelegramClient) -> bool:
        ...
        while (bot := input("You can enter a custom bot username or leave it empty and Haruka will generate a random one: ")):
            try:
                if await self._check_bot(client, bot):
                    db.set("haruka.inline", "custom_bot", bot)
    ...
    async def _badge(self, client: CustomTelegramClient):
        ...
        try:
            ...
            badge_caption = (
                ...
                ' href="https://github.com/fuckramochka/haruka/commit/{}">{}</a></b>\n🔎'
                " <b>Update status: {}</b>\n<b>{}</b>".format(
                    ".".join(list(map(str, list(__version__)))),
                    build,
                    build[:7],
            ...
            log_chat = logging.getLogger().handlers[0].get_logid_by_client(client.tg_id)
            if not log_chat:
                logging.debug(
                    "Skipping startup badge for %s: log chat is not initialized yet",
                    client.tg_id,
                )
                return
            try:
                await client.haruka_inline.bot.send_photo(
                    log_chat,
                    "https://raw.githubusercontent.com/coddrago/assets/refs/heads/main/haruka/haruka_started.png",
                    caption=badge_caption,
                )
            except Exception as e:
                logging.debug(
                    "Failed to send badge photo, falling back to text: %s",
                    e,
                )
                await client.haruka_inline.bot.send_message(log_chat, badge_caption)

            logging.debug(
                "· Started for %s · Prefix: «%s» ·",
                client.tg_id,
                client.haruka_db.get(__name__, "command_prefix", False) or ".",
            )
        except Exception:
    ...
    async def amain(self, first: bool, client: CustomTelegramClient):
        ...
        await client.start()
        client.haruka_background_ready = asyncio.Event()
        ...
        client.haruka_background_ready.set()
    ...
    def main(self):
        ...
        try:
            ...
            if str(e) == "Event loop stopped before Future completed.":
                logging.info("Main loop was stopped during shutdown.")
            else:
                logging.exception("Unexpected runtime error in main loop: %s", e)
        except Exception as e:
...
harukatl.extensions.html.CUSTOM_EMOJIS = not get_config_key("disable_custom_emojis")
