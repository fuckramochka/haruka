...
from harukatl.tl.types import DialogFilter, TextWithEntities, Message
...
class UpdaterMod(loader.Module):
    ...
    async def _set_autoupdate_state(self, call: BotInlineCall, state: bool):
        ...
        await call.answer(self.strings("autoupdate_on"), show_alert=True)
        await call.delete()
    ...
    async def _send_photo_notice(
        ...
        try:
            ...
            logger.debug(
                "Failed to send updater photo %s: %s. Falling back to text",
                photo,
                e,
            )
    ...
    def _repo_slug(self) -> str:
        origin = str(self.config["GIT_ORIGIN_URL"] or "").strip()
        ...
        return "fuckramochka/haruka"

    def _compare_link(self, current: str, upcoming: str, label: str) -> str:
        return (
            f'<a href="https://github.com/{self._repo_slug()}/compare/'
            f'{current}...{upcoming}">{label}</a>'
        )

    def _get_remote_state_sync(self) -> typing.Tuple[str, str]:
        try:
            repo = git.Repo()
    ...
    def _format_changelog(self, diff: str) -> str:
        if not diff:
            return ""
        ...
        if diff.count("\n") >= 10:
            changelog += self.strings("more").format(len(diff.splitlines()) - 10)

        return changelog

    def get_changelog(self) -> str:
        diff, _ = self._get_remote_state_sync()
        return self._format_changelog(diff) or False

    def _get_latest_sync(self) -> str:
        try:
            repo = git.Repo()
            for remote in repo.remotes:
                remote.fetch()
    ...
    def get_latest(self) -> str:
        return self._get_latest_sync()

    @loader.loop(interval=300, wait_before=True, autostart=True)
    async def poller(self):
        if self.config["disable_notifications"] and not self.config["autoupdate"]:
            raise loader.StopLoop

        if self._poller_lock.locked():
            return

        async with self._poller_lock:
            diff, latest = await asyncio.to_thread(self._get_remote_state_sync)

        if not diff or not latest:
            return

        changelog = self._format_changelog(diff)
        self._pending = latest

        if self.get("ignore_permanent", False) == self._pending:
            return
        ...
        if self._pending not in {utils.get_git_hash(), self._notified}:
            if not self.config["autoupdate"]:
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        r = await session.get(
                            url=(
                                f"https://api.github.com/repos/{self._repo_slug()}/"
                                f"contents/haruka/version.py?ref={version.branch}"
                            ),
            ...
            current = (utils.get_git_hash() or "")[:12]
            pending = self._pending[:12]
            compare_link = self._compare_link(
                current,
                pending,
                self._pending[:6] if manual_update else "🔎 diff",
            )

            if manual_update:
                update_notice = await self._send_photo_notice(
                    self.tg_id,
                    photo="https://raw.githubusercontent.com/coddrago/assets/refs/heads/main/haruka/updated.png",
                    caption=self.strings("update_required").format(
                        (utils.get_git_hash() or "")[:6],
                        compare_link,
                        changelog,
                    ),
                ...
                self.set("upd_msg", update_notice.message_id)

            else:
                await self._send_photo_notice(
                    self.tg_id,
                    photo="https://raw.githubusercontent.com/coddrago/assets/refs/heads/main/haruka/updated.png",
    ...
    async def update_call(self, call: InlineCall):
        ...
        if call.data == "haruka/ignore_upd":
            latest = self._pending or await asyncio.to_thread(self._get_latest_sync)
    ...
    async def changelog(self, message: Message):
        ...
        if (await self._client.get_me()).premium:
            changelog.replace('🌑 Haruka', '<emoji document_id=5192765204898783881>🌘</emoji><emoji document_id=5195311729663286630>🌘</emoji><emoji document_id=5195045669324201904>🌘</emoji>')
            ...
            branch_ref = (
                next(
                    (
                        ...
                        if ref.remote_head == version.branch
                    ),
                    None,
                )
                or next((ref for ref in origin.refs if ref.remote_head == "main"), None)
                or next((ref for ref in origin.refs if ref.remote_head == "master"), None)
            )
            if branch_ref is None:
                raise RuntimeError("No target branch in origin remote")

            local_branch = branch_ref.remote_head
            repo.create_head(local_branch, branch_ref)
            repo.heads[local_branch].set_tracking_branch(branch_ref)
            repo.heads[local_branch].checkout(True)
            return False
    ...
    async def update(self, message: Message):
        try:
            args = utils.get_args_raw(message)
            current = utils.get_git_hash() or "unknown"
            upcoming = await asyncio.to_thread(self._get_latest_sync)
            if not upcoming:
                upcoming = current
            if (
                "-f" in args
    ...
    async def client_ready(self):
        try:
            ...
            logger.debug(
                "Git repository is unavailable. Updater notifications are disabled "
                "until repository is initialized."
            )

        if self.config["disable_notifications"] is None:
            self.config["disable_notifications"] = True

        self._markup = lambda: self.inline.generate_markup(
            [
                {"text": self.strings("update"), "data": "haruka/update"},
                {"text": self.strings("ignore"), "data": "haruka/ignore_upd"},
            ]
        )
    ...
    async def _post_start_client_ready(self):
        ...
        try:
            ...
            logger.exception("Updater post-start initialization failed")

    async def _add_folder(self):
        folders = await self._client(GetDialogFiltersRequest())

        if any(getattr(folder, "title", None) == "haruka" for folder in folders.filters):
            return
                    ...
                    DialogFilter(
                        ...
                        include_peers=[
                            ...
                            and (
                                ...
                                and dialog.name in {"haruka-logs", "silent-tags"}
                            )
                            or (
