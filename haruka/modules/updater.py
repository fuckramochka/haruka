...
from telethon.tl.types import DialogFilter, Message, TextWithEntities
...
def _repo_url(config) -> str:
    """User-configurable repository URL of this Haruka distribution"""
    url = str(config["GIT_ORIGIN_URL"] or DEFAULT_REPO_URL).strip().rstrip("/")
    return url or DEFAULT_REPO_URL


def _repo_slug(config) -> str:
    """`owner/repo` part of the configured repository URL"""
    return _repo_url(config).replace("https://github.com/", "").strip("/")


def _announcement_url() -> typing.Optional[str]:
    """Announcement source, overridable via config.json (`announcement_url`)"""
    return (
        main.get_config_key("announcement_url")
        or "https://api.github.com/repos/coddrago/assets/contents/haruka/announcment.txt"
    )
        ...
        if self._pending not in {utils.get_git_hash(), self._notified}:
            if not self.config["autoupdate"]:
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        r = await session.get(
                            url=f"https://api.github.com/repos/{_repo_slug(self.config)}/contents/haruka/version.py?ref={version.branch}",
                            headers={"Accept": "application/vnd.github.v3.raw"},
                        )
                        text = await r.text()
            ...
            if manual_update:
                m = await self.inline.bot.send_photo(
                    ...
                    caption=self.strings("update_required").format(
                        ...
                        '<a href="{}/compare/{}...{}">{}</a>'.format(
                            _repo_url(self.config),
