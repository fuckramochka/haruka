    ...
    async def client_ready(self):
        ...
        self.text = (
            lambda: self.strings("base").format(
                ...
                if self.client.haruka_me.premium is True
                else "Haruka"
            )
            + (
        ...
        try:
            ...
            existing_channel_id = self.db.get("haruka.forums", "channel_id", None)
            ...
            if not content_channel:
                async for dialog in self.client.iter_dialogs():
                    if dialog.title and "haruka-userbot" in dialog.title.lower():
                        content_channel = dialog.entity
                        logger.debug(
                            f"Found existing channel '{dialog.title}' with ID {dialog.entity.id}"
                        )
                        self.db.set(
                            "haruka.forums", "channel_id", int(dialog.entity.id)
                        )
                        break
            ...
            if not content_channel:
                ...
                self.db.set("haruka.forums", "channel_id", int(content_channel.id))

            if not content_channel:
                raise RuntimeError("Failed to get or create content channel!")

            forum_entity = None
            existing_forum_id = self.db.get("haruka.forums", "forum_id", None)
            ...
            if not forum_entity:
                try:
                    if not (
                        ...
                        from harukatl.tl.functions.channels import ToggleForumRequest
                    ...
                    self.db.set("haruka.forums", "forum_id", int(content_channel.id))
                except Exception as e:
                    forum_entity = content_channel
            ...
            required_topics = [
                (
                    "Assets",
                    "🌆 Your Haruka assets will be stored here",
                    5877307202888273539,
                ),
                (
                    "Logs",
            ...
            logger.exception(
                ...
                "- Non working inline-logs, assets features\n"
