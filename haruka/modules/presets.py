...
class Presets(loader.Module):
    """Suggests new Haruka users a packs of modules to load"""
    ...
    async def _menu(self):
        markup = self.inline.generate_markup(self._markup)
        try:
            ...
            logger.debug(
                "Failed to send presets photo, falling back to text: %s",
                e,
            )
    ...
    async def presets(self, message: Message):
        ...
        try:
            await self.inline.form(
                message=message,
                photo='https://raw.githubusercontent.com/coddrago/assets/refs/heads/main/haruka/presets_cmd.png',
                text=text,
                reply_markup=self._markup,
            )
        except Exception as e:
            logger.debug(
                "Failed to render presets form with photo, retrying without it: %s",
                e,
            )
            await self.inline.form(
                message=message,
