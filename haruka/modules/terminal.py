...
class MessageEditor:
    def __init__(
        self,
        message: harukatl.tl.types.Message,
        command: str,
        config,
        strings,
        request_message,
    ...
    async def redraw(self):
        ...
        with contextlib.suppress(harukatl.errors.rpcerrorlist.MessageNotModifiedError):
            try:
                self.message = await utils.answer(self.message, text)
            except harukatl.errors.rpcerrorlist.MessageTooLongError as e:
                logger.error(e)
                logger.error(text)
...
class SudoMessageEditor(MessageEditor):
    ...
    async def update_stderr(self, stderr):
        ...
        if any(lastlines[0] == i for i in self.PASS_REQ) and self.state == 0:
            ...
            text = self.strings("auth_needed").format(self.message.client.haruka_me.id)

            try:
                await utils.answer(self.message, text)
            except harukatl.errors.rpcerrorlist.MessageNotModifiedError as e:
                logger.debug(e)
            ...
            self.message.client.add_event_handler(
                ...
                harukatl.events.messageedited.MessageEdited(chats=["me"]),
            )
    ...
    async def on_message_edited(self, message):
        ...
        if hash_msg(message) == hash_msg(self.authmsg):
            ...
            except harukatl.errors.rpcerrorlist.MessageNotModifiedError:
...
class RawMessageEditor(SudoMessageEditor):
    ...
    async def redraw(self):
        ...
        with contextlib.suppress(
            harukatl.errors.rpcerrorlist.MessageNotModifiedError,
            harukatl.errors.rpcerrorlist.MessageEmptyError,
            ValueError,
        ):
            try:
                await utils.answer(self.message, text)
