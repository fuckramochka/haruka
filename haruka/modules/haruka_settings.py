...
@loader.tds
class HarukaSettingsMod(loader.Module):
    """Advanced settings for Haruka Userbot"""

    strings = {"name": "HarukaSettings"}
    ...
    async def _uninstall(self, call: InlineCall):
        ...
        if any(folder.title == "haruka" for folder in folders):
            folder_id = max(
                folders,
                key=lambda x: x.id,
            ).id
    ...
    @loader.command()
    async def uninstall_haruka(self, message: Message):
    ...
    async def inline__setting(self, call: InlineCall, key: str, state: bool = False):
        if callable(key):
            key()
            harukatl.extensions.html.CUSTOM_EMOJIS = not main.get_config_key(
    ...
    async def invokecmd(self, message: Message):
        ...
        if module == "core":
            if method == "flush_entity_cache":
                result = (
                    f"Dropped {len(self._client._haruka_entity_cache)} cache records"
                )
                self._client._haruka_entity_cache = {}
            elif method == "flush_fulluser_cache":
                result = (
                    f"Dropped {len(self._client._haruka_fulluser_cache)} cache records"
                )
                self._client._haruka_fulluser_cache = {}
            elif method == "flush_fullchannel_cache":
                result = (
                    f"Dropped {len(self._client._haruka_fullchannel_cache)} cache"
                    " records"
                )
                self._client._haruka_fullchannel_cache = {}
            elif method == "flush_perms_cache":
                result = f"Dropped {len(self._client._haruka_perms_cache)} cache records"
                self._client._haruka_perms_cache = {}
                ...
                result = (
                    f"Dropped {len(self._client._haruka_entity_cache)} entity cache"
                    " records\nDropped"
                    f" {len(self._client._haruka_fulluser_cache)} fulluser cache"
                    " records\nDropped"
                    f" {len(self._client._haruka_fullchannel_cache)} fullchannel cache"
                    " records\nDropped"
                ...
                self._client._haruka_fulluser_cache = {}
                self._client._haruka_fullchannel_cache = {}
                self._client.haruka_me = await self._client.get_me()
                ...
                result = (
                    "Entity cache:"
                    f" {len(self._client._haruka_entity_cache)} records\nFulluser cache:"
                    f" {len(self._client._haruka_fulluser_cache)} records\nFullchannel"
                    " cache:"
                    f" {len(self._client._haruka_fullchannel_cache)} records\nLoader"
