...
class InlineStuff(loader.Module):
    ...
    @loader.command()
    async def ch_haruka_bot(self, message: Message):
        ...
        self._db.set("haruka.inline", "custom_bot", args)
    ...
    async def ch_bot_token(self, message: Message):
        ...
        if not args or not re.match(r"[0-9]{8,10}:[a-zA-Z0-9_-]{34,36}", args):
            ...
            return
        self._db.set("haruka.inline", "bot_token", args)
        await utils.answer(message, self.strings("bot_updated"))
        ...
        match message.text:
            case "/start":
                await message.answer_photo(
                    "https://raw.githubusercontent.com/coddrago/assets/refs/heads/main/haruka/start_cmd.png",
                    caption=self.strings("this_is_haruka").format(
                        (
                            "<tg-emoji emoji-id=5463379725441341739>🪐</tg-emoji>"
                            if self._client.haruka_me.premium
                            else "🪐"
                        ),
                        utils.get_platform_emoji() if self._client else "Haruka",
                    ),
                    reply_markup=self.inline.generate_markup(
                        markup_obj=[
                            [
                                {
                                    "text": "GitHub",
                                    "url": "https://github.com/coddrago/Haruka",
    ...
    async def reset_prefix(self, call: InlineCall, message):
        ...
        self.db.set("haruka.main", "command_prefix", ".")
