...
@loader.tds
class HarukaWebMod(loader.Module):
    """Web/Inline mode add account"""

    strings = {"name": "HarukaWeb"}
    ...
    async def weburl(self, message: Message, force: bool = False):
        ...
        if "SHARKHOST" in os.environ or "HIKKAHOST" in os.environ or "JAMHOST" in os.environ:
            ...
            if not main.haruka.web:
                main.haruka.web = core.Web(
                    data_root=main.BASE_DIR,
                    api_token=main.haruka.api_token,
                    proxy=main.haruka.proxy,
                    connection=main.haruka.conn,
                )
                await main.haruka.web.add_loader(self._client, self.allmodules, self._db)
                await main.haruka.web.start_if_ready(
                    len(self.allclients),
                    main.haruka.arguments.port,
                    proxy_pass=main.haruka.arguments.proxy_pass,
                )
    ...
    def _get_client(self) -> CustomTelegramClient:
        return CustomTelegramClient(
            MemorySession(),
            main.haruka.api_token.ID,
            main.haruka.api_token.HASH,
            connection=main.haruka.conn,
            proxy=main.haruka.proxy,
    ...
    async def schedule_restart(self, call, client):
        ...
        await main.haruka.save_client_session(client, delay_restart=False)
        restart()
