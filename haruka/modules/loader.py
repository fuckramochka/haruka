...
import requests
try:
    from harukatl.errors.common import ScamDetectionError
except Exception:
    class ScamDetectionError(Exception):
        pass
from harukatl.errors.rpcerrorlist import MediaCaptionTooLongError
from harukatl.tl.functions.channels import JoinChannelRequest
from harukatl.tl.types import Channel, Message
...
class LoaderMod(loader.Module):
    ...
    async def _run_background_startup_jobs(self):
        await self._wait_background_ready()
        try:
            await self._update_modules()
            await self._async_init()
        except Exception:
            logger.exception("Background loader startup jobs failed")
    ...
    async def client_ready(self):
        ...
        main.haruka.ready.set()

        asyncio.ensure_future(self._run_background_startup_jobs())
    ...
    async def download_and_install(
        ...
        *,
        ...
        try:
            ...
            try:
                r, changed = await self._storage.fetch(
                    url,
            ...
            if skip_unchanged and not changed:
                logger.debug(
                    "Using cached source for %s: module source not modified",
                    url,
                )
    ...
    async def _refresh_remote_modules(self, modules: typing.Iterable[str]):
        for mod in modules:
            with contextlib.suppress(Exception):
                await self.download_and_install(mod, skip_unchanged=True)
    ...
    async def approve_internal(
        ...
        await call.edit(
            ...
            photo="https://raw.githubusercontent.com/coddrago/assets/refs/heads/main/haruka/joined_jr.png",
        )
    ...
    async def load_module(
        ...
        if re.search(r"# ?scope: ?haruka_min", doc):
            ver = re.search(r"# ?scope: ?haruka_min ((?:\d+\.){2}\d+)", doc).group(1)
        ...
        if not did_requires:
            requirements = loader.parse_requirements(doc)
        ...
        module_name = f"haruka.modules.{uid}"
        doc = geek.compat(doc)
        ...
        try:
            try:
                ...
                requirement = loader.normalize_requirement(e.name)
                ...
                if not self.allmodules.secure_boot:
                    self.set(
                        "loaded_modules",
                        {
                            ...
                            if mod != instance.__class__.__name__
                        },
                    )
        ...
        try:
            try:
                ...
                async def inner_proxy():
                    ...
                    while True:
                        if hasattr(instance, "haruka_wait_channel_approve"):
                            if message:
                                (
                                    module,
                                    channel,
                                    reason,
                                ) = instance.haruka_wait_channel_approve
                                message = await utils.answer(
                                    message,
        ...
        try:
            developer_entity = await (
                ...
                if (
                    developer in self._client.haruka_entity_cache
                    and getattr(
    ...
    async def _update_modules(self):
        ...
        if self._db.get(loader.__name__, "secure_boot", False):
            ...
            for mod in todo.values():
                await self.download_and_install(mod, prefer_local=True)

            asyncio.ensure_future(self._refresh_remote_modules(todo.values()))
