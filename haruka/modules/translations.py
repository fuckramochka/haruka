...
class Translations(loader.Module):
    ...
    @loader.command(alias="lang", aliases=["lang"])
    async def setlang(self, message: Message):
        if not (args := utils.get_args_raw(message).lower()):
            ...
            if not shown:
                langs = ", ".join(sorted(translations.SUPPORTED_LANGUAGES.keys()))
                await utils.answer(
                    message,
                    (
                        f"{self.strings('choose_language')}\n\n"
                        f"<b>Supported:</b> <code>{utils.escape_html(langs)}</code>\n"
                        f"<b>Usage:</b> <code>{self.get_prefix()}setlang ua</code>"
                    ),
                    parse_mode="html",
                )
            return
