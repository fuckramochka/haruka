...
class TokenObtainment(InlineUnit):
    async def _create_bot(
        ...
        try:
            ...
            form.add_field(
                "file",
                open(f"{os.getcwd()}/assets/haruka.png", "rb"),
                filename="haruka.png",
        ...
        data = {
            "title": f"🪐 Haruka {utils.get_version_raw()}"[:64],
            "username": username,
            "about": "",
            "userpic": photo_id,
            "method": "createBot",
        ...
        async with session.post(
            ...
            if content.get("error", False) == "NEWBOT_LIMIT_EXCEEDED":
                logger.error(
                    ...
                    "command ch_haruka_bot @username"
                )
                return False
    ...
    async def _assert_token(
        ...
        username = self._db.get("haruka.inline", "custom_bot", False)
        if username:
