...
from harukatl.errors.rpcerrorlist import (
...
from ..types import HarukaReplyMarkup
...
class Form(InlineUnit):
    async def form(
        ...
        *,
        ...
        :param manual_security: By default, Haruka will try to inherit inline buttons security from the caller (command)
                                If you want to avoid this, pass `manual_security=True`
        :param disable_security: By default, Haruka will try to inherit inline buttons security from the caller (command)
        ...
        with contextlib.suppress(AttributeError):
            _haruka_client_id_logging_tag = copy.copy(self._client.tg_id)  # noqa: F841
            ...
            try:
                status_message = await (
                    ...
                    (
                        ...
                        if self._client.haruka_me.premium and CUSTOM_EMOJIS
                        else "🪐"
                    )
        ...
        try:
            ...
            form = self._units.get(unit_id, {})
            has_external_media = any(
                key in form for key in ("photo", "gif", "video", "file", "audio")
            )

            if not has_external_media:
                return await _invoke_failed()

            logger.warning(
                "Inline media fetch failed for form %s, retrying without media",
                unit_id,
                exc_info=True,
            )

            for key in ("photo", "gif", "video", "file", "audio", "mime_type"):
                form.pop(key, None)
                ...
                await inline_query.answer(
                    [
                        InlineQueryResultAudio(
                            ...
                            caption=form.get("text"),
                            parse_mode="HTML",
                            title=form["audio"].get("title", "Haruka"),
