...
### 🛠 Fixes
- **Dispatcher no longer reacts to non-commands**: any dotted text that is not a
  registered command (e.g. `...`, `.random words`) is silently ignored.
- Fuzzy "did you mean" suggestions are now **opt-in** (db key `suggest_commands`,
  default **off**) and only fire for single-word inputs with a close match
  (cutoff 0.70, throttled to once per 30 s).
- Startup badge: neutral status icon when git is disabled.
- AFK module: bounded memory for the notification cache.

## 🌸 Haruka 3.2.0 — New Identity & Feature Wave

### 🎨 Visual identity (no longer looks like Heroku)
- **New console banner**: colored "HARUKA" ASCII art with sakura tagline.
- **New logo** (`assets/haruka.png`): generated sakura-night monogram — used as the inline bot avatar and web favicon (local, no external redirects).
- **New startup badge** in log chat: compact tree-style status message, no external photos.
- **New web palette** "sakura night": accent `#ff7eb6`, secondary `#9d7bff`, success `#2dd4a0`.

### ✨ Features
- **Fuzzy command suggestions**: typo in a command? The bot answers "did you mean …?" with the closest matches (toggle via db key `suggest_commands`).
- **New core modules**:
  - `.afk [reason]` — AFK mode with auto-replies, anti-flood (1 reply per user per 5 min), state survives restart;
  - `.note/.notes/.delnote` — quick personal notes;
  - `.undo [N]` — delete your last N messages in a chat.
- **`--dry-run`**: offline validation of config + core module syntax without connecting.
- **`/healthz`** liveness endpoint on the web server + Docker healthcheck in `docker-compose.yml`.
- **uvloop** is enabled automatically on Linux/macOS when installed.
- **Security scan**: external modules are statically analyzed on load (AST) — dangerous patterns (`os.system`, `ctypes`, `eval`) are logged as warnings and attached to the module (`__security_findings__`).
- **`tools/newmodule.py`**: scaffolder for new modules (`python tools/newmodule.py MyFeature [--dir loaded_modules]`).

## 🪐 Haruka 3.1.0 — Independence & Compatibility Release

### 🔓 Independence
- **Kill-switch removed**: the bot no longer fetches a remote "allowed ids" list on startup and **always starts**, even fully offline / with an unreachable upstream. Haruka is now self-governed.
- Update source is configurable: change `GIT_ORIGIN_URL` in `.updater` config (or `HARUKA_REPO_URL` environment variable) to point at any fork — update checks, compare links and version polling follow it automatically.
- Announcement source overridable via `config.json` key `"announcement_url"`.
- Added `pyproject.toml` — the bot can be installed with pip (`pip install .`) instead of git-only workflows.

### 🧩 Universal module compatibility (`haruka.compat`)
- New meta-path import finder: modules written for **Hikka** (`import hikka...`, `import hikkatl...`), **Heroku** (`import heroku...`, `import herokutl...`) and FTG/GeekTG-style forks now resolve transparently to Haruka/stock-Telethon equivalents — both via plain `import` and `importlib.import_module`.
- Legacy client attribute aliases are kept in sync automatically: `client.hikka_me/db/inline/loader/allmodules/dispatcher` and `client.heroku_*` mirror their `haruka_*` counterparts.
- Database auto-migration extended: keys from `hikka.*`, `heroku.*` and `legacy.*` owners are converted to `haruka.*` on load.

### 🩺 New: Diagnostics core module
- `.health` (alias `.diag`): uptime, version, Python/Telethon versions, CPU/RAM usage, event-loop lag indicator, GC stats, loaded modules/commands/watchers counts, entity cache size.
- `.gcstats`: force garbage collection and report freed memory.
- `tools/selfcheck.py`: offline self-check tool (10 checks: compat layer, DB migration, updater overrides, hashing).
- `install.bat`: one-click Windows installer/launcher.

### 🛠 Fixes & improvements
- Fixed `CacheRecordFullChannel.__hash__` / `CacheRecordFullUser.__hash__` crash (referenced non-existent attributes).
- Dependency auto-install hint for legacy Telethon forks now suggests `telethon>=1.44` instead of the retired `Haruka-TL-New`.
- Removed UTF-8 BOMs left by earlier bulk edits; cleaned `__pycache__` artifacts and stray log files.
- Safer, non-greedy database key migration regex.
...
- remove "haruka" from bot username
- fix asset channel
...
- remove invokecmd
- remove uninstall_harukacmd
- remove debugmodcmd
- add new validator RandomLink
- replace validators in help, harukainfo, tester to RandomLink
...
- added config for emoji in config (.cfg harukaconfig)
...
- Telethon has been completely updated, moving to haruka-tl
- update haruka-tl to 1.1.0
- added prefix to ping
- removed official support serv00, tothost, goorm, railway, termux
- rename folder "hikka" to "haruka"
- fix close btn
- fix native imports
- added converting Hikka backups to Haruka
- updated to aiogram 3.20.0
- fix some bugs with buttons in inline

## 🪐 Haruka 1.6.8

- Finally renamed to Haruka and new Emoji
...
- added config for show "Haruka" in info
- removed UnitHeta
...
- https://github.com/coddrago/Haruka/issues/115
- https://github.com/coddrago/Haruka/issues/114
- https://github.com/coddrago/Haruka/issues/119
- https://github.com/coddrago/Haruka/issues/120
- Other changes
...
- Drop Okteto support
- Show Haruka platform and version in Telegram sessions list
...
- Completely drop Haruka support due to legacy code, limits and removing of free tier
...
- Publish hikka telethon fork and migrate to it in requirements, thereby fixing the deployment error on Haruka
- Add custom emojis filter to `utils.remove_html`
- Fix `client.get_perms_cached`
- Fix translation flaw in `HarukaSecurity`
...
- Add platform-specific Haruka emojis to premium users
...
- Add support for multiaccounting on Haruka
...
- Remove `haruka3` from classic requirements, along with haruka installation code snippet
...
- Update quickstart
- New fields in HarukaInfo
- Remove suffix `Haruka` if user specified it somewhere below
...
- Fix haruka-specific config error
...
- Add automatic webpage bot unblock in haruka waker
- Add secure boot feature
- Update native haruka postgre database saving method
...
- Add `banner_url` config var to `HarukaInfo`
- Add `loader.validators.Hidden`
- Add `websockets` dependency, so users can load hikarichat on Haruka
...
- Add `client.force_get_entity` to bypass Haruka Cacher
...
- Fix translation issue in HarukaConfig
- Fix `.dump`
...
- Add additional Haruka deps
- Fix `load_module` reattempt
- Reorder database read-writes to make postgres the ladder
- Make redis optional for non-haruka users
- Add source to Pipfile
- New Haruka dependencies list in Pipfile
...
- Add banner with Haruka installation status
- Reorder config saving in web
- Fix `Unauthorized` error on Haruka
...
- Update gitignore so git doesn't count shit on haruka
- Visual haruka fixes in updater
- Deepsource fixes
- Add `utils.get_entity_url`, `utils.get_message_link`, `utils.remove_html`, `utils.get_kwargs`
- Disable modules debugging on haruka
...
- Refactor `haruka.py` app searching algorithm
- Refactor postgresql database saving process
- Fix haruka restart message not being edited
- Add haruka waker
- Make `hikka-` app naming optional

## 🌑 Haruka 1.2.0

- Add full-featured Haruka support with additional buildpacks
...
- Drop Uniborg and Raphielgang compatibility layer. More info in [2d525ab](https://github.com/hikariatama/Haruka/commit/2d525ab6b0e6b9d7ccd7408bbe175cea24d780a5)
- Minor bug fixes

## 🌑 Haruka 1.1.0

- Make inline commands' docstrings translatable
- Introducing HarukaDynamicTranslate - the possibility for developers to translate their own modules to other language without usage of native translates
...
- Refactor HarukaSecurity, especially the bounding mask control
...
- Add ability to disable HarukaDL natively
- Fix Updater so it works if you have troubles with inline mode

## 🌑 Haruka 1.0.22

Add HarukaDL module to use inline download buttons in verified channels
