...
def install_dependencies(executable: Path) -> None:
    if dependencies_ready(executable):
        log("✓", "Dependencies are already installed and unchanged; skipping download.")
        return
...
def launch(executable: Path) -> None:
    log("✓", "Starting Haruka. The browser opens only when account setup is required.")
...
def main() -> None:
    ...
    try:
        ...
        if args.doctor or not was_ready:
            doctor(executable)
            log("✓", "Installation and diagnostics completed successfully.")
        else:
            log("✓", "Existing installation is healthy; skipped package downloads and full diagnostics.")
